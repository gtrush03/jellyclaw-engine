...
function handleMessageStart(
  ...
  if (usage) {
    // Accumulate input tokens across turns. Each message_start brings new
    // input for that turn (user prompt, tool results, etc.).
...
function handleMessageDelta(
  ...
  if (usage) {
    // Accumulate output tokens. message_delta carries the output delta for
    // this turn (tokens generated by the assistant).
    const output = num(usage.output_tokens);
    if (output !== undefined) state.outputTokens += output;
    // Note: input/cache tokens in message_delta echo message_start values for
    // this turn (not deltas). We don't re-add them to avoid double-counting.
    // Accumulation of input tokens happens in handleMessageStart.
  }
  const delta = isRecord(chunk.delta) ? chunk.delta : undefined;
