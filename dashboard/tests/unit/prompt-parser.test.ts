...
describe('parsePromptFile', () => {
  ...
  it('handles missing frontmatter gracefully', async () => {
    ...
    const fixturePath = path.join(
      REPO_ROOT,
      'prompts/phase-00/99-unit-test-fixture-no-fm.md',
    );
    await fs.writeFile(
      fixturePath,
  ...
  it('handles special characters in content', async () => {
    const fixturePath = path.join(
      REPO_ROOT,
      'prompts/phase-00/98-unit-test-fixture-special.md',
    );
    await fs.writeFile(
      fixturePath,
  ...
  it('refuses paths outside the repo root (traversal guard)', async () => {
    await expect(parsePromptFile('/etc/passwd')).rejects.toThrow(
...
describe('extractPhaseName', () => {
  it('pulls the phase name from the canonical H1 format', () => {
    expect(
      extractPhaseName('Phase 01 — OpenCode Pinning — Prompt 02: implement'),
    ).toBe('OpenCode Pinning');
  });
